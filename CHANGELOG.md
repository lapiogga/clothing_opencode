...
## [2026-02-20] - 재고 이력 미반영 문제 수정 및 화면 타이틀 통일

### 수정 (Fixed)
- **재고 이력 미반영 문제** (`order_service.py`)
  - 온라인 주문 수령(`receive_order`) 시 재고 차감 및 이력 기록 추가
  - 온라인 주문 생성 시 재고 예약(`_reserve_inventory`) 추가
  - 온라인 주문 취소 시 재고 예약 해제(`_release_reserved_inventory`) 추가
  - 기존: 오프라인 판매만 재고 이력 기록 → 수정: 온라인 주문도 재고 이력 기록

- **화면 타이틀과 메뉴명 불일치 수정**
  - Refund.vue: "반품 처리" → "판매현황"
  - ClothingList.vue: "품목 관리" → "피복 품목 관리"
  - OrderList.vue: "주문 관리" → "온라인 주문 관리"
  - Stats.vue: "판매 통계" → "통계"
  - Profile.vue: "내 정보" → "프로필"

### 재고 처리 흐름 (수정 후)

**온라인 주문:**
1. 주문 생성 → 예약 포인트 증가, **예약 재고(reserved_quantity) 증가**
2. 주문 취소 → 예약 포인트 해제, **예약 재고 해제**
3. 주문 수령 → 포인트 확정 차감, **실재고 차감 + 이력 기록**

**오프라인 판매:**
1. 판매 → 포인트 증시 차감, 실재고 차감 + 이력 기록 (기존과 동일)
